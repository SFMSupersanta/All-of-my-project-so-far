...
/***************************************************************
////////////////////////////////////////////////////////////////
This problem is moderately more difficult than the above problems.  This one requires a data type that stores 12 digits and involves a further decomposition.

Background

A Universal Product Code (UPC) bar code consists of 12-digits.  The first 6 digits represent the company that produces the product, the next 5 digits represent a unique item number that that company has assigned to the product and the last digit is a check digit.  The check digit is used to determine if the code has been properly interpreted by the scanning device. 

To determine the expected value of the check digit, use the weighted sum of the other digits:
...
the expected check digit is the digit that when added to the weighted total results in a number that is divisible by 10
...
Design a program that validates a UPC and displays the company and product numbers separately.  Your program keeps accepting a whole number and determining if that whole number is a valid UPC.  Your program terminates when the user enters 0 as the whole number. 
